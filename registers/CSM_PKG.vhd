...
package CSM_CTRL is
  type CSM_LPGBT_UPLINK_UPLINK_MON_t is record
    READY                      :std_logic;     -- LPGBT Uplink Ready
    FEC_ERR_CNT                :std_logic_vector(15 downto 0);  -- Data Corrected Count
  end record CSM_LPGBT_UPLINK_UPLINK_MON_t;
  type CSM_LPGBT_UPLINK_UPLINK_MON_t_ARRAY is array(0 to 1) of CSM_LPGBT_UPLINK_UPLINK_MON_t;

  type CSM_LPGBT_UPLINK_UPLINK_CTRL_t is record
    RESET                      :std_logic;     -- Reset this Uplink
  end record CSM_LPGBT_UPLINK_UPLINK_CTRL_t;
  type CSM_LPGBT_UPLINK_UPLINK_CTRL_t_ARRAY is array(0 to 1) of CSM_LPGBT_UPLINK_UPLINK_CTRL_t;

  constant DEFAULT_CSM_LPGBT_UPLINK_UPLINK_CTRL_t : CSM_LPGBT_UPLINK_UPLINK_CTRL_t := (
                                                                                       RESET => '0'
                                                                                      );
  type CSM_LPGBT_UPLINK_MON_t is record
    UPLINK                     :CSM_LPGBT_UPLINK_UPLINK_MON_t_ARRAY;
  end record CSM_LPGBT_UPLINK_MON_t;


  type CSM_LPGBT_UPLINK_CTRL_t is record
    UPLINK                     :CSM_LPGBT_UPLINK_UPLINK_CTRL_t_ARRAY;
  end record CSM_LPGBT_UPLINK_CTRL_t;


  constant DEFAULT_CSM_LPGBT_UPLINK_CTRL_t : CSM_LPGBT_UPLINK_CTRL_t := (
                                                                         UPLINK => (others => DEFAULT_CSM_LPGBT_UPLINK_UPLINK_CTRL_t )
                                                                        );
  type CSM_LPGBT_DOWNLINK_MON_t is record
    READY                      :std_logic;     -- LPGBT Downlink Ready
  end record CSM_LPGBT_DOWNLINK_MON_t;


  type CSM_LPGBT_DOWNLINK_CTRL_t is record
    RESET                      :std_logic;     -- Reset this Downlink
  end record CSM_LPGBT_DOWNLINK_CTRL_t;


  constant DEFAULT_CSM_LPGBT_DOWNLINK_CTRL_t : CSM_LPGBT_DOWNLINK_CTRL_t := (
                                                                             RESET => '0'
                                                                            );
  type CSM_LPGBT_MON_t is record
    UPLINK                     :CSM_LPGBT_UPLINK_MON_t;
    DOWNLINK                   :CSM_LPGBT_DOWNLINK_MON_t;
  end record CSM_LPGBT_MON_t;


  type CSM_LPGBT_CTRL_t is record
    UPLINK                     :CSM_LPGBT_UPLINK_CTRL_t;
    DOWNLINK                   :CSM_LPGBT_DOWNLINK_CTRL_t;
  end record CSM_LPGBT_CTRL_t;


  constant DEFAULT_CSM_LPGBT_CTRL_t : CSM_LPGBT_CTRL_t := (
                                                           UPLINK => DEFAULT_CSM_LPGBT_UPLINK_CTRL_t,
                                                           DOWNLINK => DEFAULT_CSM_LPGBT_DOWNLINK_CTRL_t
                                                          );
  type CSM_SC_MASTER_RX_RX_MON_t is record
  ...
  end record CSM_SC_MASTER_RX_RX_MON_t;
  type CSM_SC_MASTER_RX_RX_MON_t_ARRAY is array(0 to 2) of CSM_SC_MASTER_RX_RX_MON_t;

  type CSM_SC_MASTER_RX_MON_t is record
    RX                         :CSM_SC_MASTER_RX_RX_MON_t_ARRAY;
  end record CSM_SC_MASTER_RX_MON_t;


  type CSM_SC_MASTER_MON_t is record
    ...
    RX                         :CSM_SC_MASTER_RX_MON_t;       
  end record CSM_SC_MASTER_MON_t;


  type CSM_SC_MASTER_CTRL_t is record
  ...
  end record CSM_SC_MASTER_CTRL_t;


  constant DEFAULT_CSM_SC_MASTER_CTRL_t : CSM_SC_MASTER_CTRL_t := (
                                                                   TX_RESET => '0',
                                                                   RX_RESET => '0',
                                                                   TX_START_WRITE => '0',
                                                                   TX_START_READ => '0',
  ...
  type CSM_SC_SLAVE_MON_t is record
    RX_DATA_FROM_GBTX          :std_logic_vector( 7 downto 0);  -- Data from the FIFO
    TX_READY                   :std_logic;                      -- IC core ready for a transaction
    RX_EMPTY                   :std_logic;                      -- Rx FIFO is empty (no reply from GBTx)
  end record CSM_SC_SLAVE_MON_t;


  type CSM_SC_SLAVE_CTRL_t is record
    TX_RESET                   :std_logic;     -- Reset TX datapath
    RX_RESET                   :std_logic;     -- Reset RX datapath
    TX_START_WRITE             :std_logic;     -- Request a write config to the GBTx (IC)
    TX_START_READ              :std_logic;     -- Request a read config to the GBTx (IC)
    ...
    TX_WR                      :std_logic;                      -- Request a write operation into the internal FIFO (Data to GBTx)
    RX_RD                      :std_logic;                      -- Request a read operation of the internal FIFO (GBTx reply)
  end record CSM_SC_SLAVE_CTRL_t;


  constant DEFAULT_CSM_SC_SLAVE_CTRL_t : CSM_SC_SLAVE_CTRL_t := (
                                                                 TX_RESET => '0',
                                                                 RX_RESET => '0',
                                                                 TX_START_WRITE => '0',
                                                                 TX_START_READ => '0',
  ...
  type CSM_SC_MON_t is record
    MASTER                     :CSM_SC_MASTER_MON_t;
    SLAVE                      :CSM_SC_SLAVE_MON_t; 
  end record CSM_SC_MON_t;


  type CSM_SC_CTRL_t is record
    MASTER                     :CSM_SC_MASTER_CTRL_t;
    SLAVE                      :CSM_SC_SLAVE_CTRL_t; 
  end record CSM_SC_CTRL_t;


  constant DEFAULT_CSM_SC_CTRL_t : CSM_SC_CTRL_t := (
                                                     MASTER => DEFAULT_CSM_SC_MASTER_CTRL_t,
                                                     SLAVE => DEFAULT_CSM_SC_SLAVE_CTRL_t
                                                    );
  type CSM_MON_t is record
    LPGBT                      :CSM_LPGBT_MON_t;
    SC                         :CSM_SC_MON_t;   
  end record CSM_MON_t;


  type CSM_CTRL_t is record
    LPGBT                      :CSM_LPGBT_CTRL_t;
    SC                         :CSM_SC_CTRL_t;   
  end record CSM_CTRL_t;


  constant DEFAULT_CSM_CTRL_t : CSM_CTRL_t := (
                                               LPGBT => DEFAULT_CSM_LPGBT_CTRL_t,
                                               SC => DEFAULT_CSM_SC_CTRL_t
                                              );


end package CSM_CTRL;